...
export default function SpotifyTracker() {
  ...
  const addMember = async () => {
    ...
    const newMember = { name: newMemberName.trim(), createdAt: Date.now() };
    try {
        ...
        {activeTab === 0 && (
          <div className="space-y-4 animate-in fade-in duration-300">
            ...
            <div className="flex gap-2 mb-6">
              <input type="text" value={newMemberName} onChange={(e) => setNewMemberName(e.target.value)} placeholder="Nuevo integrante..." className="flex-1 p-3 border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-green-500 outline-none" onKeyDown={(e) => e.key === 'Enter' && addMember()} />
              <button onClick={addMember} disabled={!newMemberName.trim()} className="bg-green-600 text-white p-3 rounded-lg hover:bg-green-700 shadow-md transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                  ...
                  {memberToDelete === member.id ? (
                    <div className="flex items-center justify-between bg-red-50 p-2 rounded -m-2 animate-in slide-in-from-right-2">
                      ...
                      <div className="flex gap-2 shrink-0">
                         ...
                         <button onClick={() => confirmRemoveMember(member.id)} className="px-3 py-1 text-xs font-bold text-white bg-red-600 rounded hover:bg-red-700 shadow-sm">Sí</button>
                      </div>
                    </div>
                  ) : (
                     ...
                     {isConfirmingUndo ? (
                        <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-xl flex flex-col items-center justify-center gap-3 animate-in zoom-in-95 duration-200 shadow-sm">
                           ...
                           <div className="flex gap-3 w-full">
                              ...
                              <button onClick={() => confirmUndoPayment(member.id)} className="flex-1 py-2 bg-yellow-500 text-white rounded-lg font-bold shadow-sm hover:bg-yellow-600">Revertir</button>
                           </div>
                        </div>
                     ) : (
                        <div className="flex items-center gap-3 group">
                          <button onClick={() => !isPaid && markAsPaid(member)} disabled={isPaid} className={`flex-1 p-5 rounded-xl shadow-md text-left transition-all transform active:scale-[0.98] flex justify-between items-center relative overflow-hidden ${isPaid ? 'bg-gray-100 text-gray-400 cursor-default border border-gray-200' : 'bg-gradient-to-r from-green-500 to-green-600 text-white hover:shadow-lg hover:to-green-500 border-b-4 border-green-700'}`}>
                            <span className="font-bold text-xl truncate pr-2 z-10 relative">{member.name}</span>
                            {isPaid ? <span className="flex items-center gap-1 text-sm font-bold bg-gray-200 text-gray-500 px-3 py-1 rounded-full shrink-0 z-10 shadow-sm border border-gray-300">PAGADO <Check className="w-4 h-4" /></span> : <span className="text-xs sm:text-sm bg-black bg-opacity-20 px-3 py-1 rounded-full shrink-0 z-10 backdrop-blur-sm">Tocar para cobrar</span>}
                          </button>
                          {isPaid && <button onClick={() => setPaymentToUndo(member.id)} className="p-4 bg-white text-yellow-500 rounded-xl border border-gray-200 hover:bg-yellow-50 hover:text-yellow-600 hover:border-yellow-200 transition-all shadow-sm group-hover:shadow-md" title="Revertir pago"><RotateCcw className="w-6 h-6" /></button>}
                        </div>
                     )}
                  </div>
                );
              ...
              {showMonthGrid && (
                <div className="grid grid-cols-3 gap-2 p-3 bg-gray-50 border-t border-gray-100 animate-in slide-in-from-top-2">
                  {monthNamesList.map((mName, index) => (
                    <button key={mName} onClick={() => selectSpecificMonth(index)} className={`py-3 text-sm font-medium rounded-lg transition-all ${currentDate.getMonth() === index ? 'bg-green-600 text-white shadow-md scale-105' : 'bg-white text-gray-600 hover:bg-green-100 border border-gray-200'}`}>{mName}</button>
                  ))}
                </div>
              )}
            </div>
