...
import { Calendar, ChevronLeft, ChevronRight, Users, AlertCircle, Check, RotateCcw } from 'lucide-react';
...
export function PaymentList({ members, payments, currentDate, onChangeMonth, onMarkAsPaid, onUndoPayment }: PaymentListProps) {
                        ...
                        <div key={member.id} className="relative transition-all duration-300">
                            {isConfirmingUndo ? (
                                ...
                                <div className="flex items-center gap-3 group">
                                    <button
                                        onClick={() => !isPaid && onMarkAsPaid(member, key)}
                                        disabled={isPaid}
                                        className={`flex-1 p-5 rounded-xl shadow-md text-left transition-all transform active:scale-[0.98] flex justify-between items-center relative overflow-hidden ${isPaid ? 'bg-gray-100 text-gray-400 cursor-default border border-gray-200' : 'bg-gradient-to-r from-green-500 to-green-600 text-white hover:shadow-lg hover:to-green-500 border-b-4 border-green-700'}`}
                                    >
